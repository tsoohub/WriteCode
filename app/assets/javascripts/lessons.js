...
$(function() {
	...
	var arr = document.getElementById("total_chapter").innerText;

	arr = arr.substring(1); // Эхний [ устгаж байна
	arr = arr.slice(0,-1); // Дараагийн ] устгаж байна
	var total = arr.split(', ');


	for(var i=0; i < total.length; i++){
		$("#diagram-id-"+total[i].toString()).circleDiagram();
	}
});
...
$(document).on('page:change', function () {
	$("#view_popover").on('click', function(){
		$("#first_popover").popover("show");
	});

	// Хариулт харах panel дээр дарсахн
	$("#subscore").on('click', function(){
		// Хичээлийн онооноос нийт оноог хасаж байна
		var real_score = parseInt($("#pscore").text()) - parseInt($("#sscore").text());

		$("#lesson_score").val(real_score);
...
function hide_popover(param)
{
	$("#"+param).popover("hide");
}

function next_popover(next, now){
	$("#"+now).popover("hide");
	$("#"+next).popover("show");
}